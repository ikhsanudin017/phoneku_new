      ...
      <div class="bg-white">
        <div class="container mx-auto px-4 sm:px-8 md:px-16 lg:px-32 py-16">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <!-- Kiri (Form) -->
            <div class="bg-gradient-to-br from-blue-500 to-cyan-400 rounded-3xl px-6 sm:px-12 md:px-24 py-12 flex flex-col text-white overflow-hidden shadow-lg">
              <div>
                <h4 class="font-bold text-xl sm:text-2xl">Hubungi Kami</h4>
                <p class="mt-4 sm:mt-6 text-sm sm:text-base">
                  Tim kami akan dengan senang hati merespons setiap pertanyaan atau kebutuhan Anda secepat mungkin.
                </p>
              </div>

              <form @submit.prevent="handleSubmit" class="mt-10 space-y-4">
                <input
                  v-model="form.name"
                  type="text"
                ...
                <textarea
                  ...
                  placeholder="Pesan"
                  required
                  class="w-full h-32 px-0 py-3 bg-transparent border-b border-white/50 placeholder-white focus:outline-none resize-none"
                ></textarea>

                <div v-if="success" class="bg-green-100 bg-opacity-20 text-white px-4 py-3 rounded">
                  {{ success }}
                </div>

                <div v-if="error" class="bg-red-100 bg-opacity-20 text-white px-4 py-3 rounded">
                  {{ error }}
                </div>
                ...
                <button
                  type="submit"
                  :disabled="loading"
                  class="bg-white text-gray-800 font-medium mt-8 px-6 py-3 rounded-full hover:bg-gray-100 w-full sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <span v-if="loading" class="flex items-center justify-center">
                    <div class="animate-spin rounded-full h-5 w-5 border-2 border-gray-800 border-t-transparent mr-2"></div>
                    Mengirim...
                  </span>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    success.value = 'Pesan Anda telah berhasil dikirim! Tim kami akan merespon dalam 24 jam.'
    ...
    error.value = 'Terjadi kesalahan saat mengirim pesan. Silahkan coba lagi.'
