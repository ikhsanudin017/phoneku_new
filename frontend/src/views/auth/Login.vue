<template>
  <div class="flex min-h-screen font-sans bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div
      class="w-full md:w-[45%] ml-auto bg-gradient-to-br from-violet-600 via-blue-500 to-cyan-400 flex items-center justify-center p-8 relative overflow-hidden"
    >
      <!-- Animated Background Elements -->
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute -inset-[10px] opacity-50">
          <div
            class="w-40 h-40 rounded-full bg-purple-500/30 blur-3xl absolute -top-20 -left-20 animate-blob animation-delay-0"
          ></div>
          <div
            class="w-48 h-48 rounded-full bg-blue-500/30 blur-3xl absolute top-60 -right-20 animate-blob animation-delay-2000"
          ></div>
          <div
            class="w-44 h-44 rounded-full bg-cyan-500/30 blur-3xl absolute -bottom-20 left-40 animate-blob animation-delay-4000"
          ></div>
        </div>
      </div>
      ...
      <div class="w-full max-w-md transform transition-all relative">
        ...
        <div class="backdrop-blur-sm bg-white/10 rounded-2xl p-8 shadow-2xl border border-white/20">
          <div class="text-center mb-8">
            <h2
              class="text-4xl font-extrabold mb-3 drop-shadow-lg tracking-tight bg-gradient-to-r from-white via-blue-100 to-white bg-clip-text text-transparent"
            >
              Masuk
            </h2>
          ...
          <div
            v-if="error"
            class="bg-red-100/90 backdrop-blur-sm border border-red-400 text-red-700 px-4 py-3 rounded-lg mb-6 shadow-lg"
          >
          ...
          <form @submit.prevent="handleLogin" class="space-y-5">
            ...
            <div class="group">
              <label for="email" class="block text-blue-50 mb-2 text-sm font-medium tracking-wide"
                >Email</label
              >
              <div class="relative">
                <input
                ...
                <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                  <svg
                    ...
                    class="h-5 w-5 text-blue-200/70"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
                    <path
            ...
            <div class="group">
              <label
                for="password"
                class="block text-blue-50 mb-2 text-sm font-medium tracking-wide"
                >Password</label
              >
              <div class="relative">
                <input
            ...
            >
              <span v-if="loading" class="flex items-center justify-center">
                <div
                  class="animate-spin rounded-full h-5 w-5 border-2 border-violet-600 border-t-transparent mr-2"
                ></div>
                Masuk...
              </span>
...
const handleLogin = async () => {
  if (!csrfToken.value) {
    error.value = "CSRF token not initialized. Please try again.";
    return;
  }
