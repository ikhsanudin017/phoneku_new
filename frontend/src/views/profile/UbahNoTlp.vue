    ...
    <div class="container mx-auto px-4 py-8 -mt-48 relative z-10">
      <div class="bg-white rounded-xl p-6 shadow-md max-w-3xl mx-auto">
        <div class="text-center mb-8">
          <h2 class="text-2xl font-bold text-gray-900">Ubah Nomor Telepon</h2>
          <p class="text-gray-600">Update nomor telepon Anda dengan verifikasi SMS</p>
        </div>

        <!-- Current Phone -->
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
          <div class="text-center">
            <h3 class="text-sm font-medium text-gray-700 mb-1">Nomor Telepon Saat Ini</h3>
            <p class="text-lg text-gray-900">{{ currentPhone || 'Belum ditambahkan' }}</p>
          </div>
        </div>
        ...
        <div v-if="step === 1">
          <form @submit.prevent="sendSMS" class="space-y-6">
            <div>
              <label for="new_phone" class="block text-sm font-medium text-gray-700 mb-2">Nomor Telepon Baru</label>
              <input
                id="new_phone"
              ...
              />
              <p class="text-sm text-gray-500 mt-1">Harap sertakan kode negara (misalnya, +62 untuk Indonesia)</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Masukkan kata sandi Anda saat ini untuk verifikasi"
              />
            </div>
            ...
            <div class="flex justify-end space-x-4">
              ...
              >
                ...
                {{ loading ? 'Mengirim SMS...' : 'Kirim Kode Verifikasi' }}
              </button>
            </div>
          </form>
        </div>
        ...
        <div v-if="step === 2">
          <div class="mb-6">
            <div class="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded">
              Kami telah mengirimkan kode verifikasi ke <strong>{{ form.new_phone }}</strong>. Harap periksa SMS Anda dan masukkan kode di bawah ini.
            </div>
          </div>
          ...
          <form @submit.prevent="verifyOTP" class="space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="000000"
              />
              <p class="text-sm text-gray-500 mt-1">Masukkan kode 6 digit yang dikirim melalui SMS</p>
            </div>
            ...
            <div class="flex justify-between items-center">
              ...
              <div class="flex space-x-4">
                ...
                >
                  ...
                  {{ loading ? 'Memverifikasi...' : 'Verifikasi & Perbarui Telepon' }}
                </button>
              </div>
            </div>
          </form>
...
const fetchCurrentPhone = async () => {
  try {
    ...
    if (response.data.success && response.data.data.profile) {
      currentPhone.value = response.data.data.profile.phone || 'Not set'
    }
  } catch (error) {
...
const sendSMS = async () => {
  ...
  try {
    const response = await userAPI.changePhoneRequest({
...
const verifyOTP = async () => {
  ...
  try {
    const response = await userAPI.changePhoneVerify({
      new_phone: form.new_phone,
      otp: form.otp
    })
    ...
    if (response.data.success) {
      ...
      currentPhone.value = form.new_phone
      ...
      setTimeout(() => {
        ...
        router.push('/profile/tentang-saya')
      }, 2000)
    } else {
